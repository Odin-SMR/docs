...
blackbody.
The Rayleigh-Jeans correspondce (valid when \(h\nu/kT\)<<1) reads
\begin{equation}
 B_{v}=\frac{2\nu^{2}kT}{c^{2}}.
\end{equation}
...
P_{\nu} = kT.
\end{equation}
For a theoretical reciever and channel of bandwidth \(\Delta \nu\), with zero loss and gain 
...
thus the approximation in eq.~\ref{eq:skybeam} results in negligable error.
...
In practise, it seems to be a small inbalance between 
...
calibrated calibrated \smr\ spectra, using the calibration scheme decribed in Sect.~\ref{sec:calscheme}, 
can be described by
\begin{equation}
...
\label{eq:raderror}
\end{equation}
for measurements with a close to blank backgroud (i.e. \(T_{a,i}\approx\)0), uncertainties in the gain estimate 
...
\end{equation}
and the terms on the tight hande side is (1) the radiometer noise contribution,
(2) interpolated reference noise contribution, and (3) interploated gain noise contribution.  
...
In practise there is also a finite error due to non-linear and non-captured
...
\label{sec:gainvar}
In the preceeding section we derived an error for reference noise contribution.
%\begin{equation}
...
the calibration target temperature and main beam effciency. 
...
\lcomment{BR}{add description of how this variable can be used to estimate boradband 
