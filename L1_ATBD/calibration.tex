...
\item Main beam efficiency uncertainty. The main beam efficiency is estimated
 for each scan and this estimate introduce a finite error on the calibrated spectrum.
 We have that
 \begin{equation}
 ...
 \end{equation}
 There is no temperature sensor on \smr\ that measure the baffle temperature. For this reason, a constant
 value of 300\,K is used in calibration process.
 For  observations against a blank backgrund the error
...
However, it is possible that our interpretation 
of this \(T_{sp}\) is not correct, which will lead to an error
...
\end{equation}
A calibrated spectrum is proportional to the inverse of the main beam efficiency,
...
Since the main beam efficiency error is proportional to \(T_{a}\), it has low impact
...
An efficiency factor \(\eta\) for each sub-band is then calculated (from radiometric noise equation) as
\begin{equation}
