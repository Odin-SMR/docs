...
Background theory and nn overview of the \smr\ L2 processing is found
Chapter~2. The software tool that handles the tasks directly associated with
atmospheric radiative transfer and sensor modelling is described in Chapter~3,
while the actual data extraction step is covered by Chapter~4. The L2 data are
not complete without a careful characterisation of spatial resolution and
errors, which is the topic of Chapter~5. Finally, Chapter 6 gives a summary,
with focus on the most important points to correctly understand the \smr\ L2
data. A list of used acronyms is found directly after the table of contents.
...
the frequency resolution is only 2\,MHz\todo{Or will we skip Hanning?}. To
...
requires treatment of Zeeman effects have given these data low priority. This
...
accuracy better than ?\,MHz\todo{Add value.}. The retrieval
...
temperature, and is presently estimated to not exceed ?\,K\todo{Add value.}\ at
