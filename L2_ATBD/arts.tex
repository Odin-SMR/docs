...
values, in order to test that the code not only runs, but also that the actual simulation
results have not changed beyond some specified tolerance.
...
geoid, and the radius of the this geoid is set to ?\todo{Fixed or as curvature
  radius?}. The upper limit of the model atmosphere is defind in \ARTS\ by the
...
sensor responses vary linearly between the points were data are
available.\todo{Will the FillGrid method be used?}\ The variables controlling
\SnsMtr\ and \MpiVct\ are discussed in Sec.~\ref{sec:b}.
