...
%
\lcomment{PE}{How do we distribute the full characterisations? Take the stuff
...
\lcomment{PE}{How do we define the database of full characteristics?
  ...
  steps of 20\degree?). The database should cover all frequency bands.}
...
are known. 
\\
\lcomment{PE}{Or do we just report the sub-matrices along the diagonal? We
  include the interference error between the quantities, and to
  estimate/compensate for internal smoothing error, only the diagonal
  sub-matrices should suffice.}


\subsection{Observation error covariance matrix, \aCvrMtr{\RtrErr_o}}
\label{sec:Sreterr}
%
...
\end{displaymath}
The \aCvrMtr{\RtrErr_o}\ covariance matrix can be divided into sub-matrices
...
quantities. As any covariance matrix, \aCvrMtr{n}\ is symmetric (which is not
