...
considerations around radiative transfer are covered by this ATBD. For a
...
(LTE) can be assumed, even for raditaive transfer in the mesosphere and lower
...
in the microwave region for all species, besides oxygen as this molecule is
affected by the Zeeman effect. That is, Earth's magnetic field induces
polarised absorption and emission, and a matrix-vector equivalent to
Eq.~\ref{eq:rteq} is required to handle oxygen \citep[see e.g.][]{larsson:zeema:14}.

Evaluation of Eq.~\ref{eq:rteq} gives the monochromatic intensity (radiance)
along pencil-beam propgation paths, matching infinite frequency and angular
...
Eq.~\ref{eq:rteq} must be handled by all forward models for passive microwave
...
fully with the demands listed above, and the description of \ARTS\ is continued
in Chapter~\ref{chapter:arts}.
