...
As discussed in \citet{rodgers:00}, OEM can be derived in several ways. The
...
Formally, \aCvrMtr{a} shall be set to \aCvrMtr{\SttVct}, that is, a description
...
atmospheric physics. It could also be the case that \aCvrMtr{a}\ partly is used
as a ``tuning parameter''. The most common example is that the variances in
\aCvrMtr{a}\ are set with some margin to the true values, to maintain
...
some difference between \aSttVct{a}\ and the true mean mean value of \SttVct,
and also this systematic should be represented in \aCvrMtr{a}
...
and \aCvrMtr{a}, are further discussed in Chapter~\ref{chapter:retrieval}.
...
In contrast to many other satellite missions, no \smr\ specific forward model
