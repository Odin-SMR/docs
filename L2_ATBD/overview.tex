...
retrieval noise, respectively. These error components are discussed further
...
\subsubsection{The need of regularisation}
...
likely that \aSttVct{a}\ differs from the true mean mean value of \SttVct, and
also this systematic deviation should be represented in \aCvrMtr{a}\
...
Finally regarding \aCvrMtr{a}, the point made in Sec.~2.6 of \citep{rodgers:00}
...
Further, the setting of \aCvrMtr{o}\ is influenced by the exact choice if
...
off-set) there is a choice to make. For a perfect linear retrieval case, exact
the same result will be obtained for the target quantities, independently if
interfering factors are covered by \SttVct\ or \FrwMdlVct. However, inclusion
in \SttVct\ is to prefer as this result in that values describing the
interfering effects are obtained, which can be important diagnostic
...
difference between $\SttVct$ and $\aSttVct{a}$. If interfering species are part
